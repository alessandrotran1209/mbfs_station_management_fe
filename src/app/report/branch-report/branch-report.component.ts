...
export class BranchReportComponent implements OnInit {
  ...
  public getServerData(event?: PageEvent) {
    if (this.isSearching) {
      ...
      this.operationApiService
        .searchZoneOperations(this.id, '', '', '', '', '', '', '', page)
        .subscribe(
          (response: any) => {
            console.log(page);
  ...
  export() {
    let filename = `BaoCao_ChiNhanh_${this.id}_`;
  ...
  prefetchData() {
    this.apiService
      .prefetchSearchDataByZone(this.id)
